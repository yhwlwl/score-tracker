...
function bindPage(){
  ...
  $$('[data-copy]').forEach(b=>b.onclick=async()=>{await navigator.clipboard.writeText(b.dataset.copy);toast('已复制');});
  $('#logoutBtn')?.addEventListener('click',logout);
  bindChartTips();
}
function bindChartTips(){ const wrap=$('#chart'),tip=$('#chartTip'); if(!wrap||!tip)return; $$('circle[data-tip]',wrap).forEach(c=>{c.addEventListener('pointerenter',()=>{const svg=$('svg',wrap),r=svg.getBoundingClientRect(),vb=svg.viewBox.baseVal;tip.textContent=c.dataset.tip;tip.style.left=((c.cx.baseVal.value/vb.width)*r.width)+'px';tip.style.top=((c.cy.baseVal.value/vb.height)*r.height)+'px';tip.style.display='block'});c.addEventListener('pointerleave',()=>tip.style.display='none');}); }

function openExam(exam=null){
  const editing=!!exam; const date=exam?.exam_date||new Date().toISOString().slice(0,10);
  const modal=document.createElement('div'); modal.className='modal-backdrop'; modal.innerHTML=`<div class="modal"><div class="modal-head"><h3>${editing?'编辑 / 补录考试':'记录一次考试'}</h3><button class="close-btn" aria-label="关闭">×</button></div><div class="modal-body"><div class="form-grid"><div class="field"><label>考试名称</label><input id="examName" maxlength="60" placeholder="例如：高二上期中考试" value="${escapeHtml(exam?.name||'')}"></div><div class="field"><label>考试日期</label><input id="examDate" type="date" value="${date}"></div></div><div class="score-table"><div class="score-row header"><span>科目</span><span>目标成绩</span><span>真实成绩</span><span>满分</span></div>${SUBJECTS.map(s=>{const sc=exam?.scores?.[s]||{};return `<div class="score-row"><span class="subject-name">${s}</span><input inputmode="decimal" data-score="${s}" data-kind="target" placeholder="目标" value="${sc.target??''}"><input inputmode="decimal" data-score="${s}" data-kind="actual" placeholder="考后补录" value="${sc.actual??''}"><input inputmode="decimal" data-score="${s}" data-kind="max" placeholder="100" value="${sc.max??100}"></div>`}).join('')}</div><p class="form-note">可以只填目标成绩，也可以只填真实成绩。比如期中考试结束后先记真实成绩，期末考试提前填目标成绩，之后再回来补录。</p><div class="modal-actions"><button class="secondary cancel">取消</button><button class="primary save">${editing?'保存修改':'保存记录'}</button></div></div></div>`;
  document.body.appendChild(modal); const close=()=>modal.remove(); $('.close-btn',modal).onclick=close;$('.cancel',modal).onclick=close;modal.addEventListener('click',e=>{if(e.target===modal)close()});$('.save',modal).onclick=()=>saveExam(modal,exam?.id);setTimeout(()=>$('#examName',modal)?.focus(),60);
}
async function saveExam(modal,id){
  const btn=$('.save',modal); const name=$('#examName',modal).value.trim(),date=$('#examDate',modal).value;if(!name||!date){toast('请先填写考试名称和日期');return;}
  const scores={}; SUBJECTS.forEach(s=>scores[s]={}); $$('[data-score]',modal).forEach(i=>scores[i.dataset.score][i.dataset.kind]=i.value.trim()===''?null:Number(i.value));
  btn.disabled=true;btn.textContent='保存中…';try{await api('save_exam',{exam:{id,name,exam_date:date,scores}});await loadExams();modal.remove();render();toast('已保存');}catch(e){toast(e.message);btn.disabled=false;btn.textContent='保存记录';}
}
async function deleteExam(id){ const e=state.exams.find(x=>x.id===id); if(!confirm(`删除“${e?.name||'这次考试'}”？删除后无法恢复。`))return;try{await api('delete_exam',{examId:id});await loadExams();render();toast('已删除');}catch(err){toast(err.message)} }
