...
(function () {
  ...
  window.addEventListener('DOMContentLoaded', function () {
    if (typeof window.startRegister !== 'function' || typeof window.api !== 'function') return;
    ...
    window.startRegister = async function startRegisterCompat() {
      ...
      try {
        var data = await window.api('register');
        ...
        modal.innerHTML = '<div class="modal onboard"><div class="big-icon">✓</div><h2>账号已经准备好了</h2><p>请先保存账号，再进入成绩页面。</p>' +
          '<div class="credential"><small>用户名</small><div class="credential-row"><code>' + window.escapeHtml(data.user.username) + '</code><button class="copy-btn" data-copy="' + window.escapeHtml(data.user.username) + '">复制</button></div></div>' +
          '<div class="credential"><small>密码</small><div class="credential-row"><code>' + window.escapeHtml(data.password) + '</code><button class="copy-btn" data-copy="' + window.escapeHtml(data.password) + '">复制</button></div></div>' +
        ...
        document.querySelector('#compatSavedBtn').onclick = async function () {
          try {
            ...
            modal.remove();
            window.renderLogin('账号已创建并已登录，但页面加载失败，请刷新后重试。');
          }
        };
      } catch (e) {
        window.renderLogin(e && e.message ? e.message : '注册失败，请重试');
      }
    };
  });
})();
